...
def data_organizer_random_2d(kfold, nth_fold):
    ...
    image = nib.load('/mnt/paros_WORK/hsm/Perfusion/data/p0_full_060222.nii.gz')
    data_img = image.get_fdata()
    mask = nib.load('/mnt/paros_WORK/hsm/Perfusion/data/p0_mask_full_060222.nii.gz')
...
def cnn(fliter_num, kernel_size, kfold, nth_fold, trial_str):
    with strategy.scope():
        ...
        input_layer = keras.layers.Input(shape=(100, 100, 1))
...
image = nib.load('/mnt/paros_WORK/hsm/Perfusion/data/p0_full_071222.nii.gz')
data_img = image.get_fdata()
mask = nib.load('/mnt/paros_WORK/hsm/Perfusion/data/p0_mask_full_071222.nii.gz')
data_mask = mask.get_fdata()
...
x_val = data_norm[test_num,:,:,:,:]

y_val = data_mask[test_num,:,:,:,:]



model_path = '/mnt/paros_WORK/hsm/Perfusion/results/model_2d_fold_1_trial1_7.h5'


with strategy.scope():
    model = tf.keras.models.load_model(
        model_path, custom_objects={'dice_loss': dice_loss, 'dice_metric': dice_metric}, compile=True, options=None
    )
...
#nib.save(nif_test, '/home/alex/Stroke_multi/results/mask_test_3d_fold_' + str(nth_fold) + '_trial1_1.nii')
    ...
    nib.save(nif_img, '/mnt/paros_WORK/hsm/Perfusion/results_pred/pred_2d_fold_trial1_7.nii')
